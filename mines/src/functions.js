...
const getNeghbors = (board, row, column) => {
  const neighbors = [];
...
const safeNeighborhood = (board, row, column) => {
  ...
  return getNeghbors(board, row, column).reduce(safes, true);
};
...
const openField = (board, row, column) => {
  ...
  if (!field.opened) {
    ...
    if (field.mined) {
      ...
      getNeghbors(board, row, column).forEach((n) =>
        openField(board, n.row, n.column),
      );
    } else {
      const neighbors = getNeghbors(board, row, column);
